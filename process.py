...
from concurrent.futures import ProcessPoolExecutor                      # 这个库是用来处理密集任务的
...
def process(output_dir, wav_files, train_dir, test_dir, num_workers):
    ...
    for wav_file in wav_files[0: train_num]:
        ...
        results.append(executor.submit(
            partial(data_prepare, os.path.join(train_dir, "audio", fid), os.path.join(train_dir, "mel", fid),
                    wav_file)))
    ...
    for wav_file in wav_files[train_num: len(wav_files)]:
        ...
        names.append(fid)
        results.append(executor.submit(
            partial(data_prepare, os.path.join(test_dir, "audio", fid), os.path.join(test_dir, "mel", fid), wav_file)))
